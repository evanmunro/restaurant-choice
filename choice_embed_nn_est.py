...
@author: evanmunro
"""
from choiceModels import MultinomialLogit, NonLinearMLogit,NLMLEmbed, ChoiceCNN
...
dev_id = [70524,74523] 
test_id = [74524,78523] 
train_data = ChoiceDataset(datapath,train_id,8,choiceVar,featureCols)
#1024,8
train_loader = torch.utils.data.DataLoader(train_data, batch_size=2, shuffle=False, num_workers=0)

#model = NLMLEmbed(train_data.n_x,train_data.n_i,train_data.n_y,5) 
#model = MultinomialLogit(train_data.n_x)
model = ChoiceCNN(train_data.n_x,train_data.n_i,train_data.n_y,5) 
...
torch.save(model,"cnn_embed.torch") 
