from choiceModels import ChoiceSequence
import torch
...
test_data = ChoiceSequence(datapath,test_id,1000,choiceVar,featureCols) 
criterion = nn.NLLLoss(reduction='sum') 


test_loader = torch.utils.data.DataLoader(test_data,batch_size=1,shuffle=False,num_workers=0) 
model = torch.load('/home/users/munro/rnn_embed.torch') 
...
for data in test_loader: 
    ...
    loss = criterion(trainOut,labels.view(-1)) 
