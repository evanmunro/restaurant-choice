...
# -*- coding: utf-8 -*-
"""
Script for estimating RNN on restaurant choice data

@author: evanmunro
"""
from choiceModels import MultinomialLogit, NonLinearMLogit,NLMLEmbed, ChoiceCNN, ChoiceRNN
from choiceModels import ChoiceDataset, ChoiceSequence
import torch 
...
dev_id = [70524,74523] 
test_id = [74524,78523] 
train_data = ChoiceSequence(datapath,train_id,1024,choiceVar,featureCols)
#1024,8
train_loader = torch.utils.data.DataLoader(train_data, batch_size=1, shuffle=False, num_workers=0)

#model = NLMLEmbed(train_data.n_x,train_data.n_i,train_data.n_y,5) 
#model = MultinomialLogit(train_data.n_x)
model = ChoiceRNN(train_data.n_x,train_data.n_i,train_data.n_y,5) 
model.to(device) 
...
for epoch in range(5): 
    ...
    for data in train_loader: 
        ...
        #step of SGD 
        optimizer.step()
        torch.save(model,"rnn_embed2.torch") 
...
model.eval() 
dev_data = ChoiceSequence(datapath,dev_id,1024,choiceVar,featureCols) 
dev_loader = torch.utils.data.DataLoader(dev_data,batch_size=1,shuffle=False,num_workers=0)
